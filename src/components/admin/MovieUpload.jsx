...
export default function MovieUpload({ visible, onClose }) {
  ...
  const getUploadProgressValue = () => {
    if (!videoUploaded && uploadProgress >= 100) return "Processing...";
...
const UploadProgress = ({ width, message, visible }) => {
  if (!visible) return null;
  return (
    <div className="dark:bg-secondary bg-zinc drop-shadow-lg rounded p-3">
      <div className="relative h-3 dark:bg-dark-subtle bg-light-subtle overflow-hidden">
        <div
          style={{ width: width + "%" }}
          className="h-full absolute left-0 bg-secondary dark:bg-zinc"
        />
      </div>
